<template>
	<view class="wyb-table-box">
				...
				<view class="wyb-table-header-item" v-if="enableCheck" :style="{
					...
					<view class="wyb-table-checkbox" v-if="enableCheck === 'multiple'" @tap.stop="onCheckAllTap" :style="{
						...
						<text class="iconfont icon-check" v-show="checkAll" :style="{
								...
								paddingTop: (fontSize[1] || fontSize[0]) * 0.15 + 'px',
								fontSize: (fontSize[1] || fontSize[0]) + 'px'}" />
					</view>
				</view>
				<view ref="iosBug" class="wyb-table-header-item" v-for="(item, index) in headers" :key="item.key"
					@tap="onHeaderItemTap(index)" :style="{
					 minWidth: (item.width || defaultColWidth),
					 maxWidth: (item.width || defaultColWidth),
					 minHeight: minHeight[0] + 'rpx',
					 textAlign: textAlign,
					 justifyContent: textAlign === 'center' ? textAlign : (textAlign === 'left' ? 'flex-start' : 'flex-end'),
					 fontSize: fontSize[0] + 'px',
		...
		<view v-if="!loading" class="wyb-table-scroll-view" :style="{
			...
			<view class="wyb-table-content">
				<view class="wyb-table-content-line" v-for="(content, cIndex) in contentsSort"
					...
					@longpress="longPressGetData(content)">
					...
					<view class="wyb-table-content-item" v-for="(header, hIndex) in headers"
						@tap.stop="onContentItemTap(cIndex, hIndex)" :key="contentItemKey(header, hIndex)" :style="{
						 minWidth: (header.width || defaultColWidth),
						 maxWidth: (header.width || defaultColWidth),
						 textAlign: textAlign,
						 justifyContent: textAlign === 'center' ? textAlign : (textAlign === 'left' ? 'flex-start' : 'flex-end'),
						 fontSize: (fontSize[1] || fontSize[0]) + 'px',
		...
		<view class="wyb-table-noContent" v-if="contents.length==0">
			<text class="wyb-table-noContent_text">查无数据</text>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			autoContentBgColor() {
				return function(cIndex, hIndex) {
					...
					if (this.nextRow.indexOf(cIndex) != -1) {
						result = '#f9dddd';
					}
					// 再判断条件格式传没传值，设置条件样式
		...
		methods: {
			longPressGetData(content) {
				this.$emit('longPress', content)
			},
